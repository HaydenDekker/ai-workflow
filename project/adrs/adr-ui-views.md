...
Views are the **sole owners of service access** and the **coordinators of component state**. This is the single responsibility that separates views from components:

| Responsibility | View | Component |
|---------------|------|-----------|
| Service injection (`@Autowired`) | ✅ | ❌ |
| Reactive chain ownership (`Mono`, `Flux`) | ✅ | ❌ |
| `UI.access()` wrapping | ✅ | ❌ |
| Lifecycle scheduling (`ScheduledExecutorService`) | ✅ | ❌ |
| Data display (rendering `AgentInfo`, `LLMStatus`) | ✅ | ✅ (via constructor/data) |
| User interaction (button clicks, form input) | ✅ | ✅ |
| State coordination (triggering reload, navigating) | ✅ | ❌ (uses callbacks) |

**Components communicate via callbacks, not service calls:**

```java
// View owns service — coordinates everything
...
// Component is pure UI — just fires callbacks
public class AgentDetailDialog extends Dialog {
    private final Consumer<AgentDefinition> onSave;
    private final Consumer<String> onDelete;

    private void handleSave() {
        onSave.accept(buildDefinition());  // ✅ just data
        close();
    }

    private void handleDelete() {
        onDelete.accept(existingAgent.id());  // ✅ just the id
        close();
    }
}
```
...
3. **Testability**: Components with no service dependency are pure UI — testable with browserless tests using just `@ViewPackages` and mock callbacks, no Spring context needed.
4. **Re-entrancy**: Views can gate, cancel, or reorder service calls. Components have no visibility into concurrent operations.
...
Views own all reactive chains and must ensure all Vaadin component updates happen on the UI thread.
...
private void reloadData() {
    ...
    agentInfoService.getAllAgentInfos()
        .doFinally(signal -> UI.getCurrent().access(() -> showLoading(false)))
        .subscribe(
            agentInfos -> UI.getCurrent().access(() -> {
...
**Pattern 2: Mutation (save/delete) — queue reload after async operation**

```java
agentInfoService.deleteAgent(id)
    .subscribe(
        deletedId -> UI.getCurrent().access(() -> reloadData()),
        error -> UI.getCurrent().access(() -> Notification.show("Delete failed: " + error.getMessage()))
    );
```

> **⚠️ Reactor Gotcha — `Mono<Void>` and `Mono.empty()`**
>
> `Mono.empty()` completes without emitting a value. `subscribe(Consumer<T> onNext)` is **never invoked** because there is no `onNext` event.
>
> ```java
> // ❌ BUG — consumer never fires
> Mono.empty().subscribe(id -> System.out.println("Never called"));
>
> // ✅ FIXED — emit a value
> Mono.just(id).subscribe(id -> System.out.println("Fires with: " + id));
>
> // ✅ ALTERNATIVE — use onComplete for side effects
> Mono.empty().subscribe(
>     id -> {},  // onNext (never fires)
>     err -> {}, // onError
>     () -> System.out.println("onComplete fires")  // ✅ fires
> );
> ```
>
