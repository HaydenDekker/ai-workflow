 ...
 * <p>
 * Polls configured endpoints on schedule, persists status to database
 * via {@link LLMStatusRepository} port, and checks WARN condition (stale data).
 * <p>
 * Depends on port interfaces:
 * <ul>
 *   <li>{@link LLMHealthPort} — for health checking</li>
 *   <li>{@link LLMStatusRepository} — for status persistence</li>
 * </ul>
 *
 * @see LLMHealthPort
 * @see LLMStatusRepository
 */
@Service
public class AgentStatusService {
    ...
    private final LLMHealthPort healthPort;
    private final ObservabilityProperties observabilityProperties;

    @Value("${app.observability.warn-after-hours:1}")
    private long warnAfterHours;

    public AgentStatusService(LLMStatusRepository repository,
                              LLMHealthPort healthPort,
    ...
    public void schedulePolling() {
        ...
        LLMStatus status = healthPort.checkHealth(endpoint, model)
                .block();
        ...
        if (finalStatus.status() == HealthStatus.DOWN) {
            log.warn("LLM endpoint DOWN: {} - {}",
                    finalStatus.endpoint(),
                    finalStatus.errorMessage());
        } else if (finalStatus.status() == HealthStatus.WARN) {
     ...
     */
    public List<LLMStatusRepository.LLMStatusRecord> getCurrentStatus() {
        ...
        List<LLMStatusRepository.LLMStatusRecord> allRecords = repository.findAll();
        List<LLMStatusRepository.LLMStatusRecord> stale = allRecords.stream()
                .filter(e -> !e.endpoint().equals(configuredEndpoint))
                .toList();
        stale.forEach(record -> repository.deleteByEndpoint(record.endpoint()));
