...
Aiming to have AI act as the navigator rather than the driver of pair programming. A root directory is monitored for file events. The file is pushed into the configured agent workflow and a response is provided. The responses are stored back to the file system and trigger further events for any listening agent. Agents listen to file change events and filter for relevance using a regex.
...
- (done) dynamic configuration of agent workflow.
...
- (done) Agent Workflow Builder, to accept PromptRequest and return PromptResponse. Requires mapping stages but is clearer on the intent of each stage.
...
- (done) Update agent configuration to filter on file regex.
...
- (done) Create FileOutput stage for agent workflow builder and enable configuration to define target file type.
- (done) Update agent to output document to the prompt file template where the file name is a function of the input file url.
...
- (done) multiple workflows in configuration
...
- Trigger an agent to re-run manually. Anywhere along the agent chain should run it downstream.
